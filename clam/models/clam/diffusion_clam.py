...
from clam.models.utils.utils import DiffusionCLAMOutput
...
class DiffusionCLAM(BaseModel):
    def __init__(
        self, cfg: DictConfig, input_dim: int, la_dim: int = None, device: str = "cuda"
    ):
        ...
        self.name == "DiffusionCLAM"

        encoder = Network(**self.cfg.idm.net)
        decoder = UNet_decoder(**self.cfg.fdm.net)
        ...
        self.source_feature_decoder = nn.Linear(
            self.cfg.idm.net.feature_dim * 2, self.cfg.la_dim
        )

        self.device = device

        self.ddpm_sche = schedules(self.cfg.betas, self.cfg.n_T, device, "DDPM")
        self.ddim_sche = schedules(self.cfg.betas, self.cfg.n_T, device, "DDIM")
        self.n_T = self.cfg.n_T
    ...
    def perturb(self, x, t=None):
        ...
        sche = self.ddpm_sche
        x_noised = (
            sche["sqrtab"][t, None, None, None] * x
            + sche["sqrtmab"][t, None, None, None] * noise
        )
        return x_noised, t, noise

    def forward(self, observations, use_amp=False, **kwargs):
        # TODO: only using o_t-1, o_t for now
        x_source = observations[:, :-1]
        x_target = observations[:, 1]

        x_target = normalize_to_neg_one_to_one(x_target)
        x_noised, t, noise = self.perturb(x_target, t=None)

        mask = torch.bernoulli(torch.zeros(x_noised.shape[0]) + self.drop_prob).to(
            self.device
        )
        ...
        with autocast(enabled=use_amp):
            for i in range(x_source.shape[1]):
                x_source_features.append(self.encoder(x_source[:, i, :]))

            x_source_features = torch.cat(x_source_features, dim=1)
            la = self.source_feature_decoder(x_source_features)
            reconstructed_obs = self.decoder(x_noised, t / self.n_T, la, mask)

        return DiffusionCLAMOutput(
            noise=noise, reconstructed_obs=reconstructed_obs, la=la
        )
    ...
    def pred_eps_(self, x, t, model_args, guide_w, alpha, use_amp, clip_x=True):
        ...
        if clip_x:
            denoised = torch.clip(denoised, -1.0, 1.0)
